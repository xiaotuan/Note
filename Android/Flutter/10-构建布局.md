...
![finished lakes app that you'll build in 'Building a Layout'](./images/lakes.jpg)
...
![diagramming the rows in the lakes screenshot](./images/lakes-diagram.png)

接下来，绘制每一行。第一行称其为标题部分，有三个子项：一列文字，一个星形图标和一个数字。它的第一个子项，列，包含2行文字。 第一列占用大量空间，所以它必须包装在Expanded widget中。

![diagramming the widgets in the Title section](./images/title-section-diagram.png)

第二行称其为按钮部分，也有3个子项：每个子项都是一个包含图标和文本的列。

![diagramming the widgets in the button section](./images/button-section-diagram.png)
...
![sample layout](./images/lakes-icons.png)
![sample sample layout with visual debugging turned on](./images/lakes-icons-visual.png)
...
![node tree representing the sample layout](./images/sample-flutter-layout.png)
...
![screenshot of a white background with grey 'Hello World' text.](./images/hello-world.png)
...
![screenshot with callouts showing the row containing two children: a column and an image.](./images/pavlova-diagram.png)



左侧的Column widget树嵌套行和列。

![diagram showing a left column broken down to its sub-rows and sub-columns](./images/pavlova-left-column-diagram.png)
...
![diagram showing the main axis and cross axis for a row](./images/row-diagram.png)

![diagram showing the main axis and cross axis for a column](./images/column-diagram.png)
...
![a row showing 3 images spaced evenly in the row](./images/row-spaceevenly-visual.png)
...
![a column showing 3 images spaced evenly in the column](./images/column-visual.png)

**注意:* 如果布局太大而不适合设备，则会在受影响的边缘出现红色条纹。例如，以下截图中的行对于设备的屏幕来说太宽：

![a row that is too wide, showing a red string along the right edge](./images/layout-too-large.png)
...
![a row of 3 images with the middle image twice as wide as the others](./images/row-expanded-visual.png)
...
![a row of 3 images that are too wide, but each is constrained to take only 1/3 of the row's available space](./images/row-expanded-2-visual.png)
...
![a row of 5 stars, packed together in the middle of the row](./images/packed.png)
...
![a screenshot of the pavlova app, with the ratings and icon rows outlined in red](./images/pavlova-large-annotated.png)

红色边框部分的布局通过两个行来实现。评级行包含五颗星和评论数量。图标行包含三列图标和文本。

评级行的widget树:

![a node tree showing the widgets in the ratings row](./images/widget-tree-pavlova-rating-row.png)
...
![a node tree for the widets in the icons row](./images/widget-tree-pavlova-icon-row.png)
...
![a diagram showing that margins, borders, and padding, that surround content in a container](./images/margin-padding-border.png)
...
![a screenshot showing 2 rows, each containing 2 images; the images have grey rounded borders, and the background is a lighter grey](./images/container.png)
...
![a 3-column grid of photos](./images/gridview-extent.png)
...
![a 2 column grid with footers containing titles on a partially translucent background](./images/gridview-count-flutter-gallery.png)
...
![a ListView containing movie theaters and restaurants](./images/listview.png)
...
![a ListView containing shades of blue from the Material Design color palette](./images/listview-flutter-gallery.png)
...
![a circular avatar containing the label 'Mia B' in the lower right portion of the circle](/images/stack.png)
...
![an image with a grey gradient across the top; on top of the gradient is tools painted in white](./images/stack-flutter-gallery.png)
...
Material Components 库中的Card包含相关内容块，可以由大多数类型的widget构成，但通常与ListTile一起使用。Card有一个子项， 但它可以是支持多个子项的列，行，列表，网格或其他小部件。默认情况下，Card将其大小缩小为0像素。您可以使用[SizedBox](https://docs.flutter.io/flutter/widgets/SizedBox-class.html)来限制Card的大小。
...
![a Card containing 3 ListTiles](./images/card.png) 包含3个ListTiles并通过用SizedBox包装进行大小调整的Card。分隔线分隔第一个和第二个ListTiles。
...
![a Card containing an image and text and buttons under the image](./images/card-flutter-gallery.png)
...
![a Card containing 3 ListTiles](./images/card.png)

包含3个ListTiles的Card
**Dart code:** See [Card examples](https://flutterchina.club/tutorials/layout/#card-examples).

![3 ListTiles, each containing a pull-down button](./images/listtile-flutter-gallery.png)
