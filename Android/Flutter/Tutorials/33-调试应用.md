...
- [Dart 分析器](https://flutterchina.club/debugging/#dart-分析器)
- Dart Observatory (语句级的单步调试和分析器)
  - [`debugger()` 声明](https://flutterchina.club/debugging/#debugger-声明)
- [`print`、`debugPrint`、`flutter logs`](https://flutterchina.club/debugging/#printdebugprintflutter-logs)
- [调试模式断言](https://flutterchina.club/debugging/#调试模式断言)
- 调试应用程序层
  - [Widget 层](https://flutterchina.club/debugging/#widget-层)
  - [渲染层](https://flutterchina.club/debugging/#渲染层)
  - [层](https://flutterchina.club/debugging/#层)
  - [语义](https://flutterchina.club/debugging/#语义)
  - [调度](https://flutterchina.club/debugging/#调度)
- [可视化调试](https://flutterchina.club/debugging/#可视化调试)
- [调试动画](https://flutterchina.club/debugging/#调试动画)
- 调试性能问题
  - [衡量应用启动时间](https://flutterchina.club/debugging/#衡量应用启动时间)
  - [跟踪Dart代码性能](https://flutterchina.club/debugging/#跟踪dart代码性能)
- [Performance Overlay](https://flutterchina.club/debugging/#performance-overlay)
- [Material grid](https://flutterchina.club/debugging/#material-grid)
...
在开发过程中，强烈建议您使用Flutter的“调试”模式，有时也称为“checked”模式（译者语：Dart2.0后“checked”被废除，可以使用“strong” mode）。 如果您使用`flutter run`运行程序。在这种模式下，Dart assert语句被启用，并且Flutter框架使用它来执行许多运行时检查来验证是否违反一些不可变的规则。
...
这是一个“扁平化”的树，显示了通过各种构建函数投影的所有widget（如果你在widget树的根中调用`toStringDeepwidget`，这是你获得的树）。 你会看到很多在你的应用源代码中没有出现的widget，因为它们被框架的widget的`build()`函数插入的。例如，[`InkFeature`](https://docs.flutter.io/flutter/material/InkFeature-class.html)是Material widget的一个实现细节 。

由于debugDumpApp()当按钮从被按下变为被释放时被调用，FlatButton对象同时调用`setState()`，所以并因此将自己标记为脏。 这就是为什么如果你看转储，你会看到特定的对象标记为“dirty”。您还可以查看已注册了哪些手势监听器; 在这种情况下，一个单一的GestureDetector被列出，并且监听“tap”手势（“tap”是`TapGestureDetector`的`toStringShort`函数输出的）
...
[`debugPaintPointersEnabled`](https://docs.flutter.io/flutter/rendering/debugPaintPointersEnabled.html)标志打开一个特殊模式，任何正在点击的对象都会以深青色突出显示。 这可以帮助您确定某个对象是否以某种不正确地方式进行hit测试（Flutter检测点击的位置是否有能响应用户操作的widget）,例如，如果它实际上超出了其父项的范围，首先不会考虑通过hit测试。
