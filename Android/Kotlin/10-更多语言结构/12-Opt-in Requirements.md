...
> 要求选择加入的注解 `@RequiresOptIn` 与 `@OptIn` 是 *实验性的*。 请参阅[以下](https://www.kotlincn.net/docs/reference/opt-in-requirements.html#experimental-status-of-the-opt-in-requirements)用法详细信息。
...
如果库作者将一个库的 API 声明标记为[*要求选择加入*](https://www.kotlincn.net/docs/reference/opt-in-requirements.html#requiring-opt-in-for-api) 你应该明确同意在代码中使用它。 有多种方式可以选择加入使用此类 API，所有方法均不受技术限制。 你可以自由选择最适合自己的方式。

### 传播选择加入

在使用供第三方（库）使用的 API 时，你也可以把其选择加入的要求传播到自己的 API。 为此，请在你的 API 主体声明中添加注解 [*要求选择加入的注解*](https://www.kotlincn.net/docs/reference/opt-in-requirements.html#opt-in-requirement-annotations)。 这可以让你使用带有此注解的 API 元素。
...
在不公开其自身API的模块（例如应用程序）中，你可以选择使用 API 而无需将选择加入的要求传播到代码中。 这种情况下，请使用 [@OptIn](https://www.kotlincn.net/api/latest/jvm/stdlib/kotlin/-opt-in/index.html) 标记你的声明， 并以要求选择加入的注解作为参数：
...
如果想获得使用者使用你的模块 API 的明确同意，请创建一个注解类，作为_要求选择加入的注解_。 这个类必须使用 [@RequiresOptIn](https://www.kotlincn.net/api/latest/jvm/stdlib/kotlin/-requires-opt-in/index.html) 注解：
...
- `BINARY` [retention](https://www.kotlincn.net/api/latest/jvm/stdlib/kotlin.annotation/-annotation-retention/index.html)
- [targets](https://www.kotlincn.net/api/latest/jvm/stdlib/kotlin.annotation/-annotation-target/index.html)中没有 `EXPRESSION` 与 `FILE`
- 没有参数

选择加入的要求可以具有以下两个严格[级别](https://www.kotlincn.net/api/latest/jvm/stdlib/kotlin/-requires-opt-in/-level/index.html)之一：
...
为了让 API 用户相应地更新其模块（从代码中删除注解并重新编译）， 请将注解标记为 [`@Deprecated`](https://www.kotlincn.net/api/latest/jvm/stdlib/kotlin/-deprecated/index.html) 并在弃用 message 中提供说明。
