...
修改 `sys/frameworks/base/services/usb/java/com/android/server/usb/UsbDeviceManager.java` 文件中 `// The default USB connection mode is MTP by qty {{&&` 标记的代码：
...
public class UsbDeviceManager implements ActivityTaskManagerInternal.ScreenObserver {
    ...
    abstract static class UsbHandler extends Handler {
		...
		// The default USB connection mode is MTP by qty {{&& 
		private void restoreDefaultFunction() {
            if (android.os.SystemProperties.get("persist.sys.cts.test").equals("yes")) {
	    ...
	    //if (id != mUsbNotificationId || force) {
            if (id != SystemMessage.NOTE_USB_SUPPLYING && (id != mUsbNotificationId || force)) {//jnier modify otg line display usb device mode
