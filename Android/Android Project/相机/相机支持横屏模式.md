...
3. 修改 `sys/vendor/mediatek/proprietary/packages/apps/Camera2/host/src/com/mediatek/camera/CameraActivity.java` 文件如下代码：
   ...
   @@ -117,7 +121,7 @@ public class CameraActivity extends PermissionActivity implements IApp {
    
        private final List<OnOrientationChangeListener>
                mOnOrientationListeners = new ArrayList<>();
   -    private int mOrientation = OrientationEventListener.ORIENTATION_UNKNOWN;
   +    private int mOrientation = 270;//OrientationEventListener.ORIENTATION_UNKNOWN;
        private OrientationEventListener mOrientationListener;
        protected Uri mUri;
        protected OnThumbnailClickedListener mThumbnailClickedListener =
   @@ -170,6 +174,9 @@ public class CameraActivity extends PermissionActivity implements IApp {
   ...
   @@ -229,6 +236,16 @@ public class CameraActivity extends PermissionActivity implements IApp {
...
6. 修改 `sys/vendor/mediatek/proprietary/packages/apps/Camera2/common/src/com/mediatek/camera/common/utils/CameraUtil.java` 文件中 `rotateRotateLayoutChildView` 方法的如下代码：

   ```diff
   @@ -618,11 +618,38 @@ public final class CameraUtil {
                                             boolean animation) {
   ...
   +        // Solve the camera interface icon rotation direction is wrong by qty {{&&
   +        // if (displayRotation == 270 || displayRotation == 180) {
   +        //     if(SystemProperties.getInt("vendor.mtk.camera.app.platform.landscape", 0) == 0){
   +        //         orientation += 180;
   +        //     }
   +        // }
   +        if (displayRotation == 0 && orientation == 90) {
   +            orientation = 270;
   +        } else if (displayRotation == 180 && orientation == 270) {
   +            orientation = 270;
   +        } else if (displayRotation == 270 && orientation == 0) {
   +            orientation = orientation;
   +        } else if (displayRotation == 90 && orientation == 180) {
   +            orientation = 0;
   +        } else if (displayRotation == 180 && orientation == 180) {
   +            orientation = 0;
   +        } else if (displayRotation == 180 && orientation == 0) {
   +            orientation = 0;
   +        } else if (displayRotation == 0 && orientation == 180) {
   +            orientation = 0;
   +        } else if (displayRotation == 90 && orientation == 90) {
   +            orientation = 270;
   +        } else if (displayRotation == 180 && orientation == 90) {
   +            orientation = 270;
   +        } else if (displayRotation == 270 && orientation == 270) {
   +            orientation = 270;
   +        } else if (displayRotation == 270 || displayRotation == 180) {
                if(SystemProperties.getInt("vendor.mtk.camera.app.platform.landscape", 0) == 0){
                    orientation += 180;
                }
            }
