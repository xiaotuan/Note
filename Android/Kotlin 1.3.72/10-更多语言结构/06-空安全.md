...
- 有些数据在初始化时不一致，例如当：
  ...
  - [超类的构造函数调用一个开放成员](https://www.kotlincn.net/docs/reference/classes.html#派生类初始化顺序)，该成员在派生中类的实现使用了未初始化的状态；
- Java 互操作：
  - 企图访问[平台类型](https://www.kotlincn.net/docs/reference/java-interop.html#空安全与平台类型)的 `null` 引用的成员；
