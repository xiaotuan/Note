...
`gawk` 允许你定义自己的变量在程序代码中使用。`gawk` 自定义变量名可以是任意数目的字母、数字和下划线，但不能以数字开头。
...
`gawk` 编程语言包含了用来处理数字值的标准算数操作符。其中包括求余符号（`%`）和幂运算符（`^` 或 `**`）。

#### 3.2 在命令行上给变量赋值

也可以用 `gawk` 命令行来给程序中的变量赋值。这允许你再正常代码之外赋值，即时改变变量的值。
...
这个特性可以让你再不改变脚本代码的情况下就能够改变脚本的行为。

使用脚本命令行参数来定义变量值会有一个问题。在你设置了变量后，这个值在代码的 BEAGIN 部分不可用。

```shell
$ cat test.sh
BEGIN { print "The starting value is ", n ; FS="," }
{ print $n }
$ gawk -f test.sh n=3 data1.txt
...
可以使用 `-v` 命令行参数来解决这个问题。它允许你再 BEGIN 代码之前设定变量。在命令行上，`-v` 命令行参数必须放在脚本代码之前。

```shell
$ gawk -v n=3 -f test.sh data1.txt
