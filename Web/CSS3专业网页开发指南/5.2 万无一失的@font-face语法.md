...
第一种情况（<img class="my_markdown" src="./images/191.jpg" style="width:35px;  height: 34px; " width="20"/>）没有local值，使用的是EOT字体格式。在第二种情况下（<img src="https://cdn.ptpress.cn/pubcloud/5B0A982E/ushu/N8506/online/FBOL6c694c89d077c/Images/192.jpg" style="width:35px;  height: 35px; " width="20"/>），local()值有必需的字体名称，使用的则是OTF（OpenTypes）格式。这种情况会被IE8及其以下版本忽略，它们无法识别local()，但在所有其他浏览器上都会占据优先。
...
对于第一个local()值（<img class="my_markdown" src="./images/194.jpg" style="width:35px;  height: 34px; " width="20"/>），你可以使用PostScript名称，而对于第二个local()值（<img src="https://cdn.ptpress.cn/pubcloud/5B0A982E/ushu/N8506/online/FBOL6c694c89d077c/Images/195.jpg" style="width:35px;  height: 35px; " width="20"/>），你可以使用其系列名称（尽管列出的顺序实际上无关紧要）。此外，字体管理程序会帮助你找到可以在样式表中指定的不同名称。
